...
const DashboardPage = ({ user, setUser }) => {
    ...
    const fetchFunction = (vari, set, type, endpoint) => {
        fetch('https://healro1298.me/api/'+endpoint)
    ...
    const searchFunction = (event) => {
        ...
        if(event.key === 'Enter' && document.getElementById("expandable-search").value!=='') {
            ...
            fetch('https://healro1298.me/api/ibm/certifications/uid/'+document.getElementById("expandable-search").value)
                .then(response => response.json())
                .then(res => {
    ...
    const uploadFunction = (event) => {
        ...
        fetch('https://healro1298.me/api/import/upload', {
            method: 'POST',
            body: formData
        })
            .then(response => response.json())
