...
const DashboardPage = ({ user, setUser }) => {
    ...
    const fetchFunction = (vari, set, type, endpoint) => {
        fetch('http://192.168.1.70/api/'+endpoint)
    ...
    const searchFunction = (event) => {
        ...
        if(event.key === 'Enter' && document.getElementById("expandable-search").value!=='') {
            ...
            fetch('http://192.168.1.70/api/ibm/certifications/uid/'+document.getElementById("expandable-search").value)
                .then(response => response.json())
                .then(res => {
    ...
    const uploadFunction = (event) => {
        ...
        fetch('http://192.168.1.70/api/import/upload', {
            method: 'POST',
            body: formData
        })
            .then(response => response.json())
